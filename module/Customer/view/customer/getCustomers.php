    ...
    <div class='row'>
        ...
        <div class='col-lg-6 col-md-12 col-sm-12 col-xs-12'>
            <form style="display: inline-flex; width:100%;" method="POST" action="<?= Customer['paths']['url'] . '/customer/search' ?>">
                ...
                <input type='hidden' name='request' value='<?= 'customer/getCustomers'?>'>
                <input type='hidden' name='from' value='<?=$_SESSION['query']['getCustomers']?>'>
                <div class="form-group mx-sm-3 mb-2" style='width:80%; margin-top:5px'>
                    <input type="text" class="form-control" name="pattern" 
                    placeholder="<?= ucwords(Translator::translate('enter')) . " " . ucwords(Translator::translate('name')) . "..." ?>">
                </div>
        ...
        <table class="table table-striped table-hover text-center">
            <thead class='thead-dark'>
                <tr>
                    ...
                    <th class='display-tablet'><?= ucwords(Translator::translate('notes')) ?></th>
            ...
            <tbody>
                ...
                ?>
                    ...
                    <tr>
                        <td class='display-tablet-small' scope="row"><?= $customer['id'] ?></td>
                        ...
                        <td class='display-tablet'><?= $customer['notes'] ?></td>
                        <td class='display-mobile-small' style="width:170px;padding:5px;">
                            <form class='form-inline' action='<?= Customer['paths']['url'] . '/customer/deleteCustomer' ?>' method='post' enctype='multipart/form-data'>
                                <input type='hidden' name='token' value='<?= $token ?>'>
                                <input type="hidden" name='customer_id' value='<?= $customer['id'] ?>'>
                                <a class="btn btn-info mr-md-2" href="<?= Customer['paths']['url'] . '/customer/updateCustomer/' . $customer['id'] ?>" role="button">
                                    <?= ucwords(Translator::translate('update')) ?>
                                </a>
                                <button type="submit" name='delete_customer' class="btn btn-danger"><?= ucwords(Translator::translate('delete')) ?></button>
                            </form>
                        </td>
                    </tr>
